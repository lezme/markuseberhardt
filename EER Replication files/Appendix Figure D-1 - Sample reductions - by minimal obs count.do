...
*
******************************************************************************
*                Appendix Figure D-1 - Sample reduction 
******************************************************************************
...
cls
tsset wbcode2 year
local z "23 24 25 26 27 28 30 34 35 37 38 39 40 42 43 44"
foreach k of local z{
	**** Plain vanilla model
	qui: xtmg y demBMR  ///
		l0ddemBMR l1ddemBMR l2ddemBMR  ///
...
order model
sort CKdyn_minobs
save  "$otherdata/202010308_CK_lr_covar_reduction_BMR.dta", replace
restore
...
preserve 
use "$otherdata/202010308_CK_lr_covar_reduction_BMR.dta", clear
...
, xsize(9) ysize(4) scheme(s2mono) graphregion(color(white)) plotregion(color(white)) ///
	...
	xlabel(, labsize(small))  yscale(range(3.9 12.1)) ylabel(4(2)12) ///
	yscale(range(3.9 12.1) axis(2)) ymtick(##2) ymtick(##2, axis(2)) ///
	ytitle("Average Long-run Coefficient on Democracy", size(medsmall)  axis(2))  ///
	ylabel(4(2)12, labsize(small) grid glc(black) glw(.2) glp(shortdash) angle(0) axis(2)) /// 
	xlabel(25(5)45) xscale(range(22 46)) xmtick(##5)   ///
	...
	label(6 "") ///
	size(medsmall)) 
graph export "$texfolder/20220421_CKdyn_evolution_BMR.eps", as(eps) replace

list CKdyn_minobs_cov CKdyn_b_lr_dem_cov CKdyn_se_lr_dem_cov CKdyn_t_lr_dem_cov if (CKdyn_minobs_cov==40  ///
	| CKdyn_minobs_cov==44), noobs separator(0)
...
cls
tsset wbcode2 year
local z "24 25 26 27 28 30 31 35 36 38 40 41 42 43 44 45"
foreach k of local z{
	**** Plain vanilla model
	qui: xtmg y demCGV  ///
		l0ddemCGV l1ddemCGV l2ddemCGV ///
...
order model
sort CKdyn_minobs
save  "$otherdata/202010308_CK_lr_covar_reduction_CGV.dta", replace
restore

*** Graph CGV (Included in the paper)

preserve 
use "$otherdata/202010308_CK_lr_covar_reduction_CGV.dta", clear
...
, xsize(9) ysize(4) scheme(s2mono) graphregion(color(white)) plotregion(color(white)) ///
	...
	ytitle("Average Long-run Coefficient on Democracy", size(medsmall)) bgcolor(white) ///
	ylabel(, labsize(small) glcolor(gs14) glwidth(medthin) angle(0)) ///
	xlabel(, labsize(small))  yscale(range(5.9 12.5)) ylabel(6(2)12) ///
	yscale(range(5.9 12.5) axis(2)) ymtick(##2) ymtick(##2, axis(2)) ///
	ytitle("Average Long-run Coefficient on Democracy", size(medsmall)  axis(2))  ///
	ylabel(6(2)12, labsize(small) grid glc(black) glw(.2) glp(shortdash) angle(0) axis(2)) /// 
	xlabel(25(5)45) xscale(range(23 46)) xmtick(##5)   ///
	...
	label(6 "") ///
	size(medsmall)) 
graph export "$texfolder/202120421_CKdyn_evolution_CGV.eps", as(eps) replace

list CKdyn_minobs_cov CKdyn_b_lr_dem_cov CKdyn_se_lr_dem_cov CKdyn_t_lr_dem_cov if (CKdyn_minobs_cov==40  ///
	| CKdyn_minobs_cov==45), noobs separator(0)
...
cls
tsset wbcode2 year
local z "23 26 27 28 29 30 36 37 38 40 42 43 44 45 46 47"
foreach k of local z{
	**** Plain vanilla model
	qui: xtmg y demPS  ///
		l0ddemPS l1ddemPS l2ddemPS  ///
...
order model
sort CKdyn_minobs
save  "$otherdata/202010308_CK_lr_covar_reduction_PS.dta", replace
restore

*** Graph PS (Included in the paper)
preserve 
use "$otherdata/202010308_CK_lr_covar_reduction_PS.dta", clear
...
, xsize(9) ysize(4) scheme(s2mono) graphregion(color(white)) plotregion(color(white)) ///
	...
	ytitle("Average Long-run Coefficient on Democracy", size(medsmall)) bgcolor(white) ///
	ylabel(, labsize(small) glcolor(gs14) glwidth(medthin) angle(0)) ///
	xlabel(, labsize(small))  yscale(range(3.5 14.1)) ylabel(4(2)14) ///
	yscale(range(3.5 14.1) axis(2)) ymtick(##2) ymtick(##2, axis(2)) ///
	ytitle("Average Long-run Coefficient on Democracy", size(medsmall)  axis(2))  ///
	ylabel(4(2)14, labsize(small) grid glc(black) glw(.2) glp(shortdash) angle(0) axis(2)) /// 
	xlabel(25(5)45) xscale(range(22 48)) xmtick(##5)   ///
	...
	label(6 "") ///
	size(medsmall)) 
graph export "$texfolder/20220421_CKdyn_evolution_PS.eps", as(eps) replace

list CKdyn_minobs_cov CKdyn_b_lr_dem_cov CKdyn_se_lr_dem_cov CKdyn_t_lr_dem_cov ///
	if (CKdyn_minobs_cov==47  | CKdyn_minobs_cov==40), noobs separator(0)
