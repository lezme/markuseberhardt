...
*
******************************************************************************
*                 Figure 4 - Sample reduction exercises - Panel (b) 
******************************************************************************
...
cls
tsset wbcode2 year
forvalues k=2010(-1)1976{
	qui: xtreg y l(1/4).y dem yy* if year<=`k', fe r cluster(wbcode2)
...
preserve
keep FE23_*
gen model = _n if !missing(FE23_endyear)
drop if _n>16
order model
sort FE23_endyear
save  "$otherdata/FE_2_3_reduction_time_2022.dta", replace
restore
...
matrix ABTable2Column7=J(16,13,.)

local i = 1
cls
tsset wbcode2 year
forvalues k=2010(-1)1995{
	qui: xtabond2 y l(1/4).y dem yy*   if year<=`k',  gmmstyle(y, laglimits(2 .)) gmmstyle(dem, laglimits(1 .)) ivstyle(yy*   , p) noleveleq robust nodiffsargan
	* Min number of observations
	...
	* Dem coefficient (long-run)
	...
	di in gr "Table 2(7), Result #" in ye `i' in gr ": Completed results for end year " in ye `k' in gr ". N=" in ye e(N) in gr " obs."
...
sort AB27_endyear
drop if _n>16
save  "$otherdata/AB_2_7_reduction_time_2022.dta", replace
restore
...
***
*** HHK with four lags (ANRR: Table 2, Column 11)
***
...
forvalues k=2010(-1)1995{
	...
	bootstrap _b, seed(12345) reps(100) cluster(wbcode2) idcluster(newcl):  ///
		hhkBS y (dem L1.y L2.y L3.y L4.y) (y dem) () (yy*), ydeep(1960) ystart(1964) yfinal(`final') truncate(4)
	* Min number of observations
	...
	* Number of Countries
	...
	di in gr "Result #" in ye `i' in gr " for four lags: Completed results for end year " in ye `k' in gr ". N=" in ye e(N) in gr " obs."
...
rename HHK211_1 HHK211_endyear
...
preserve
keep HHK211_*
gen model = _n if !missing(HHK211_endyear)
order model
sort HHK211_endyear
save  "$otherdata/HHK_2_11_reduction_time_2022.dta", replace
restore
...
nlcom _b[dem]/(1-(_b[l1.y]+_b[l2.y]+_b[l3.y]+_b[l4.y]))
...
cls
tsset wbcode2 year
forvalues l=2010(-1)1995{
	qui: xtivreg2 y l(1/4).y (dem=l(1/4).instrument) yy* if year<=`l', fe cluster(wbcode2) r partial(yy*)
	* Min number of observations
	mat IVTable6Column2[`i',1]=`l'
	* Total number of observations
	...
	* Dem coefficient (long-run)
	...
	di in gr "Table 6(2), Result #" in ye `i' in gr ": Completed results for end year " in ye `l' in gr ". n=" in ye e(N) in gr " obs." 
...
preserve
keep IV62_*
gen model = _n if !missing(IV62_endyear)
drop if _n>16
order model
sort IV62_endyear
save  "$otherdata/IV_6_2_reduction_time_2022.dta", replace
restore
...
cls
use "$otherdata/FE_2_3_reduction_time_2022.dta", clear
gen FE23_t_lr_dem = FE23_b_lr_dem/FE23_se_lr_dem

merge model using "$otherdata/AB_2_7_reduction_time_2022.dta", sort
gen AB27_t_lr_dem = AB27_b_lr_dem/AB27_se_lr_dem
drop _merge*

merge model using "$otherdata/HHK_2_11_reduction_time_2022.dta", sort
gen HHK211_t_lr_dem = HHK211_b_lr_dem/HHK211_se_lr_dem
drop _merge*

merge model using "$otherdata/IV_6_2_reduction_time_2022.dta", sort
gen IV62_t_lr_dem = IV62_b_lr_dem/IV62_se_lr_dem



***
*** Democracy estimates - Panel (b)
***

twoway ///
	(connected FE23_b_lr_dem FE23_endyear, lpattern(solid) ///
		lw(.8) msymbol(O) lcolor(midblue*.75) msize(medlarge) mfcolor(white) mlcolor(black) mlwidth(thin)) ///
	(scatter FE23_b_lr_dem FE23_endyear if abs(FE23_t_lr_dem)>1.645, ///
		msymbol(O) lcolor(midblue*.75) msize(medlarge) mfcolor(midblue*.75) mlcolor(black) mlwidth(thin)) ///
	(connected AB27_b_lr_dem AB27_endyear, lpattern(dash) ///
		lw(.8) msymbol(O) lcolor(midblue*1) msize(medlarge) mfcolor(white) mlcolor(black) mlwidth(thin)) ///
	(scatter AB27_b_lr_dem AB27_endyear if abs(AB27_t_lr_dem)>1.645, ///
		msymbol(O) lcolor(midblue*1) msize(medlarge) mfcolor(midblue*1) mlcolor(black) mlwidth(thin)) ///		
	(connected HHK211_b_lr_dem HHK211_endyear, lpattern(shortdash) ///
		lw(.8) msymbol(O) lcolor(midblue*1.5) msize(medlarge) mfcolor(white) mlcolor(black) mlwidth(thin)) ///
	(scatter HHK211_b_lr_dem HHK211_endyear if abs(HHK211_t_lr_dem)>1.645, ///
		msymbol(O) lcolor(midblue*1.5) msize(medlarge) mfcolor(midblue*1.5) mlcolor(black) mlwidth(thin)) ///		
	(connected IV62_b_lr_dem IV62_endyear, ///
		lw(.8) msymbol(O) lcolor(midblue*2) msize(medlarge) mfcolor(white) mlcolor(black) mlwidth(thin)) ///
	(scatter IV62_b_lr_dem IV62_endyear if abs(IV62_t_lr_dem)>1.645, ///
		msymbol(O) lcolor(midblue*2) msize(medlarge) mfcolor(midblue*2) mlcolor(black) mlwidth(thin)) ///	
	(scatter IV62_b_lr_dem IV62_endyear, yaxis(2) msymbol(none))  ///
, xsize(10) ysize(5) scheme(s2mono) graphregion(color(white)) plotregion(color(white)) ///
	...
	xlabel(1995(1)2010) xscale(reverse)  yline(0, lpattern(dash) lw(.2))  ///
	...
	label(8 "") ///
	size(medsmall)) 
graph export "$texfolder/Comparison_para_evolution_time_full_2022.eps", as(eps) replace