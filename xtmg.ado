...
* Blackburne III and Mark W. Frank (xtpmg) and the Westerlund (2007) error correction cointegration 
* test (xtwest) written by Damiaan Persyn and Joakim Westerlund. Thanks to Kit Baum and a Stata  
* Journal reviewer for useful comments, help and support. Any remaining errors are my own.
*
***************************************************************************************************
...
version 10
      ...
      syntax varlist [if] [in] [, I(varname) T(varname) TREND noCONstant AUGment IMPose CCE ///
		FULL Level(cilevel) ROBUST RES(namelist) PRED(namelist)]
...
/* Compute country-estimates */
		tempname bols vari bbar vce  depT indT tmp tmp2 r r1 sig2 beta names yhat yhat1
		generate `r'=.
		generate `yhat'=.
		if ("`cce'" != ""){
			if ("`trend'" != ""){
